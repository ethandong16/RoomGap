...
const browserOptions={headless:false,args:['--window-size=1200,850']};
...
const cookies=(await context.cookies()).filter(c=>c.domain.endsWith('tust.edu.cn'));
if(!cookies.length)throw Error('未找到 tust.edu.cn Cookie');
await writeFile('.roomgap-auth.json',JSON.stringify(cookies),'utf8');
